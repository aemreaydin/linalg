...
TEST_F( Transform4Test, ConstructFromMat4 )
{
  ...
  Transform4 t( mat );
  EXPECT_TRUE( areMatricesEqual( t, mat ) );
}
...
TEST_F( Transform4Test, TransformComposition )
{
  ...
  t1.setTranslation( Point3{ 1.0F, 0.0F, 0.0F } );

  Transform4 t2{ Transform4::identity() };
  t2.setTranslation( Point3{ 0.0F, 1.0F, 0.0F } );
...
TEST_F( Transform4Test, InverseRoundtrip )
{
  ...
  Mat4       result = t * inv;
  EXPECT_TRUE( areMatricesEqual( result, Mat4::identity(), 1e-5F ) );
}
...
TEST_F( Transform4Test, NormalTransformation )
{
  ...
  t.setTranslation( Point3{ 10.0F, 20.0F, 30.0F } );

  Vec3 transformed = transformNormal( normal, t );
